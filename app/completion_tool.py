...
class Minal:
    ...
    def parse_arguments(self):
        ...
        try:
            ...
            temperature_arg = self.get_cli_argument(['--temperature', '-t'], config.get('temperature', 0.5))
            if temperature_arg is None:
                ...
                try:
                    ...
                    logger.error(f"Invalid temperature value: {temperature_arg}, using default value.")
                    self.temperature = 0.5

            # Handle max_tokens safely
            max_tokens_arg = self.get_cli_argument('--max_tokens', config.get('max_tokens', 100))
            if max_tokens_arg is None:
                ...
                try:
                    ...
                    logger.error(f"Invalid max tokens value: {max_tokens_arg}, using default value.")
                    self.max_tokens = 100
            ...
            self.streaming = self.get_cli_argument(['--stream', '-s'], config.get('streaming', False), is_flag=True)
            self.token_usage = self.get_cli_argument(['--token_usage'], config.get('token_usage', False), is_flag=True)
        ...
        try:
            ...
            if file_path and os.path.exists(file_path):
                ...
                if context:
                    ...
                    logger.info(f"Context: {context[:100]}")  # Log the first 100 characters of the context
                return context
            else:
                logger.error(f"File not found: {file_path}")
                return None
        ...
        try:
            ...
            try:
                ...
                if self.streaming:
                    ...
                    if self.token_usage:
                        completion_tokens, prompt_tokens = self.extract_chunk_token_usage(_response, self.provider)
    ...
    def extract_chunk_token_usage(self, response, provider):
        ...
        if provider == "OpenAI API":
            ...
            usage = getattr(response, 'usage_metadata', None) or getattr(response, 'token_usage', None)
            if usage:
